...
```bash
git clone https://github.com/mateofalco/JobTracker.git
cd JobTracker
npm install
```
...
Go to [supabase.com](https://supabase.com), create a new project, then open the SQL Editor and run this:
...
**3. Set up Google OAuth (optional — email/password works without it)**

In Google Cloud Console, create an OAuth 2.0 client ID (Web application type). Add this as an authorized redirect URI:

```
https://your-project-ref.supabase.co/auth/v1/callback
```

Then in Supabase: Authentication → Providers → Google → paste your Client ID and Secret → Enable.

**4. Get a Gemini API key**

Go to [aistudio.google.com](https://aistudio.google.com/app/apikey) and create a key. The free tier works fine for personal use.
...
Open [http://localhost:3000](http://localhost:3000). The root redirects to `/board` if you're logged in, or `/login` if not.

---

## Environment Variables

`NEXT_PUBLIC_SUPABASE_URL` — your Supabase project URL. Found in Project Settings → API. The `NEXT_PUBLIC_` prefix means it's available in the browser, which is intentional — the URL by itself doesn't grant any access.

`NEXT_PUBLIC_SUPABASE_ANON_KEY` — the Supabase anon key, also in Project Settings → API. This is also browser-safe by design. It lets users authenticate and read their own data, but Row Level Security prevents them from reading anyone else's.

`GEMINI_API_KEY` — your Google AI Studio API key. No `NEXT_PUBLIC_` prefix. This stays server-side only and is never sent to the browser.
...
2. Go to [vercel.com/new](https://vercel.com/new) and import the repository. Vercel detects Next.js automatically.
...
After deploying, update two settings in Supabase Authentication → URL Configuration:
- **Site URL**: your Vercel URL (e.g. `https://your-app.vercel.app`)
- **Redirect URLs**: `https://your-app.vercel.app/auth/callback`

If using Google OAuth, also add `https://your-app.vercel.app/auth/callback` to the authorized redirect URIs in your Google Cloud Console OAuth client.
